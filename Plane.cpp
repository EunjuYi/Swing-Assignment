...
int main() {
	...
	int money = 0; //총 금액: 예약을 나눠서 해도 합이 집계됨
	...
	do {
		...
		case 1: {
			...
			totalP = adult + child;
			//금액은 예약을 나눠서 해도 합계로 출력해야 하므로, += 사용하여 기존 money에 더해준다.
		...
		case 2: {
			...
			while (numChg > 0) {
				...
				while (true) {
					...
					//예약이 안되어 있는 경우
					if (IfReserved(fSection, sSection, tSection, currSection, currCol, currRow) != 1) {
